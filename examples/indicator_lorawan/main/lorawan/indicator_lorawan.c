...
static void OnMacMcpsRequest(LoRaMacStatus_t status, McpsReq_t *mcpsReq, TimerTime_t nextTxIn)
{
    ...
    switch (mcpsReq->Type) {
        case MCPS_CONFIRMED: {
            lorawan_log_display(LORAWAN_LOG_LEVEL_INFO, "firm Packet pwm_set = 1, status: %s\r\n", MacStatusStrings[status]);
            //PWM_SET = 1;
            break;
        }
        case MCPS_UNCONFIRMED: {
            lorawan_log_display(LORAWAN_LOG_LEVEL_INFO, "Send unconfirm Packet pwm_set = 1, status: %s\r\n", MacStatusStrings[status]);
            //PWM_SET = 1;
            break;
        }
        default: {
...
TaskHandle_t lorawanTaskHandle = NULL;//asdasd
...
int indicator_lorawan_init(void)
{
    ...
    //xTaskCreate(&__lorawan_task, "__lorawan_task", 1024 * 5, NULL, 2, &lorawanTaskHandle);

...
{
    int ret = 0;
    uint8_t cmd = 0x01; // Replace with your desired command
    uint8_t data[1] = {0x04}; // Replace with your desired data

    ret = __cmd_send(cmd, data, sizeof(data));
    if (ret <= 0) {
        ESP_LOGI(TAG, "Failed to send command to RP2040. %d", ret);
    } else {
        ESP_LOGI(TAG, "Successfully sent command to RP2040. %d", ret);
    }
}
static void __commu_event_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
